...

{
  ...
  $('.js-add-form01').click(function (e) {
    ...
    $('.quote-list__fee-add').each(function () {
      //Add data child
      $(this).find('.js-send-result01').click(function (e) {
        e.preventDefault();
        var timef = $(this).parent().find('.time-first').val();
        var timefm = $(this).parent().find('.time-first-min').val();
        var timel = $(this).parent().find('.time-last').val();
        var timelm = $(this).parent().find('.time-last-min').val();
        $(this).parent().next().find('#time-first').html(timef);
        $(this).parent().next().find('#time-first-min').html(timefm);
        $(this).parent().next().find('#time-last').html(timel);
        $(this).parent().next().find('#time-last-min').html(timelm);

        //			$(this).parent().next().find('.result-txt').text(timef + price + "～" option + price);

        if (timefm.length != 0 && timelm.length != 0) {
...
$('.js-show-accs').click(function () {
  $(this).toggleClass('on');
  $(this).next(".js-ct-accs").slideToggle();
}); //check btn page U022-2
...
/* 今日の日付をセット */
$(function () {
  if (document.querySelector('.js_today')) {
    var date = document.querySelector('.js_today');
    var today = new Date();
...
/* クレジット有効期限 */
$(function () {
  if (document.querySelector('.js-expiry')) {
    document.querySelector('.js-expiry').addEventListener('keypress', function () {
