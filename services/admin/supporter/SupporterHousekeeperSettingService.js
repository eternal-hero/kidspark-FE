...
export default {
  async getSetting(nuxtApp,id) {
    const { data } = await api.get(API.SUPPORTER_HOUSEKEEPING_SETTING.replace(':supporter_user_id:', id),{
      headers: {
  ...
  async updateSetting(nuxtApp, supporter_id, setting) {
    ...
    if(setting.id == null) {
        await api.post(API.SUPPORTER_HOUSEKEEPING_SETTING.replace(':supporter_user_id:', supporter_id), setting,{
          headers: {
            Authorization: 'Bearer ' + nuxtApp.$cookies.get('auth_token')
          }
        })
    } else {
        await api.put(API.SUPPORTER_HOUSEKEEPING_SETTING.replace(':supporter_user_id:', supporter_id), setting,{
          headers: {
            Authorization: 'Bearer ' + nuxtApp.$cookies.get('auth_token')
          }
        })
  ...
  async getSettingSupports(nuxtApp,supporter_id) {
    const { data } = await api.get(`/api/admin/supporter/${supporter_id}/housekeeping/supports`,{
      headers: {
        Authorization: 'Bearer ' + nuxtApp.$cookies.get('auth_token')
      }
    });
  ...
  async createSettingSupports(nuxtApp,supporter_id,formData) {
    const res = await api.post(`/api/admin/supporter/${supporter_id}/housekeeping/supports`, formData,{
      headers: {
        Authorization: 'Bearer ' + nuxtApp.$cookies.get('auth_token')
      }
    });
  ...
  async updateSettingSupports(nuxtApp,supporter_id,formData) {
    const res = await api.put(`/api/admin/supporter/${supporter_id}/housekeeping/supports`, formData,{
      headers: {
        Authorization: 'Bearer ' + nuxtApp.$cookies.get('auth_token')
      }
    });
